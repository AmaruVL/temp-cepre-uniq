...
const FormularioFechas = (props) => {
  ...
  function onSubmit(dataForm) {
    ...
    if (dataForm.escuelaProfesional !== -1) {
      texto = `${dataForm.ciclos}/${dataForm.escuelaProfesional}`;
    }
    setIsLoading(true);
    axios
      .get(`${RutaGeneral}/consultas/ver-preinscripciones/${texto}`)
      .then((res) => {
        ...
        <div className="p-10">
          <Controller
            ...
            render={({ field }) => (
              ...
              >
                {props.escuelasProfesionales.map((value, index) => (
                  <MenuItem key={index} value={value.id}>
                    {value.nombre_escuela_profesional}
                  </MenuItem>
                ))}
