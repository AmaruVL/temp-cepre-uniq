...
const PopperEliminar = (props) => {
  ...
  const eliminarEstado = (values) => {
    setIsLoading(true);
    axios
      .delete(`${Ruta}/ciclos/${value.id}`)
      .then((res) => {
        setIsLoading(false);
        showSuccess('Se eliminó el ciclo.');
        axios
          .get(`${Ruta}/ciclos`)
          .then((res2) => {
      ...
      .catch((err) => {
        setIsLoading(false);
        setOpen(false);
        showError('No se pudo eliminar el ciclo.');
      });
  };
      ...
      >
        <Box sx={style}>
          <Typography className="p-10">
            Esta seguro de eliminar el ciclo <b>{value.denominacion}</b> ?
          </Typography>
