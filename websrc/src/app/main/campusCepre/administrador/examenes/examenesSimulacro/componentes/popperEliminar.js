...
const PopperEliminar = (props) => {
  ...
  const eliminarEstado = (values) => {
    setIsLoading(true);
    axios
      .delete(`${Ruta}/examen-grupo/limpiar-preguntas/${value.id}`)
      .then((res) => {
        setIsLoading(false);
        showSuccess('Se eliminó el comentario.');
        axios.get(`${Ruta}/examen-grupo/ver-examenes-grupo/${idExamen}`).then((res2) => {
  ...
  return (
    <>
      <Tooltip title="Eliminar configuracion hecha para el examen.">
        <IconButton
      ...
      >
        <Box sx={style}>
          <Typography className="p-10">
            Esta seguro de eliminar la configuracion del examen{' '}
            <b>{value.id_examen.denominacion_examen}</b> ?
          </Typography>
