...
const PopperEliminarMaterialLista = (props) => {
  ...
  const eliminarEstado = (values) => {
    setIsLoading(true);
    axios
      .delete(`${Ruta}/docente/material-curso/${idHorario}/${value.id}`)
      .then((res) => {
        setIsLoading(false);
        showSuccess('Se eliminó el correctamente.');
        handleClose();
        axios.get(`${Ruta}/docente/material-curso/${idHorario}`).then((resp) => {
      ...
      >
        <Box sx={style}>
          <Typography className="p-10">
            Esta seguro de eliminar <b>{value.descripcion_material}</b> ?
          </Typography>
